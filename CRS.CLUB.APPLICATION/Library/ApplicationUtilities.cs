...
"</div> ";
        ...
        public static bool HasMenuRight(string pagePath = "")
        {
            HttpContext context = HttpContext.Current;
            if (String.IsNullOrEmpty(pagePath))
            {
            ...
            else
            {
                pagePath = pagePath.Replace("~/", "");
                pagePath = pagePath.Contains("?") ? pagePath.Split('?')[0] : pagePath;
                var functions = (context.Session["Menus"] as List<MenuModel>)?.ConvertAll(x => x?.MenuUrl?.ToUpper());
                if (functions != null)
                    if (!functions.Contains(pagePath.ToUpper()))
                    {
                        return false;
        ...
        public static object LoadDropdownList(string ForMethod, string search1 = "", string search2 = "")
        {
            ...
            switch (ForMethod.ToUpper())
            {
                ...
                case "COMMISSIONPERCENTAGETYPELIST":
                    dbResponse = _CommonBuss.GetDropDown("002", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "COMMISSIONCATEGORYLIST":
                    dbResponse = _CommonBuss.GetDropDown("003", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "CLUBLIST":
                ...
                case "CLUBCOMMISSIONDETAIL":
                    ...
                    return response;
                case "PREFECTUREDDL":
                    dbResponse = _CommonBuss.GetDropDown("015", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "PLANDDL":
                    dbResponse = _CommonBuss.GetDropDown("016", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "LOCATIONDDL":
                    dbResponse = _CommonBuss.GetDropDown("017", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "CLUBSCHEDULE":
                    ...
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "DISPLAYORDERDDL":
                    dbResponse = _CommonBuss.GetDropDown("030", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "HOSTDDLBYCLUBID":
                    dbResponse = _CommonBuss.GetDropDown("033", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
                case "SELECTCLUB":
                    dbResponse = _CommonBuss.GetDropDown("035", search1, search2);
                    dbResponse.ForEach(item => { response.Add(item.Key.EncryptParameter(), item.Value); });
                    return response;
        ...
        public static string StaticEncryptData(this string textToEncrypt)
        {
            ...
            try
            {
                if (String.IsNullOrEmpty(textToEncrypt))
                    return "";
                var encoded = cipher.Encrypt(textToEncrypt);
                var replacedString = encoded.Replace("/", "bb1234565Abcd").Replace("+", "574352674GGHGbvsb").Replace("=", "779u8jnjfsnhg32gg");
                return replacedString;
            }
            catch
            {
        ...
        public static string StaticDecryptData(this string textToDecrypt)
        {
            StringCipher cipher = new StringCipher("thisiscrsStaticEncryptDecryptData");
            try
            {
                var replacedString = textToDecrypt.Replace("bb1234565Abcd", "/").Replace("574352674GGHGbvsb", "+").Replace("779u8jnjfsnhg32gg", "=");
                return cipher.Decrypt(replacedString);
            }
            catch
            {
